...
class _TransformerDenoiser(nn.Module):
    ...
    def forward(
        ...
        x = self.token_emb(x_t) + self.pos_emb(pos)
        x = x + self.time_mlp(self._sinusoidal(t)).unsqueeze(1)
...
class AbsorbingD3PM(nn.Module):
    ...
    Vocabulary layout expected from mdiff.data.vocab.Vocabulary:
    ...
    def __init__(
        ...
        self.register_buffer("alpha_bar", (T - t_vals) / T)
    ...
    def forward(
        ...
        """Hybrid loss L_vb + λ·L_CE at masked positions."""
        B = x0.shape[0]
        ...
        logits = self.denoiser(x_t, t, pad_mask)

        if not is_masked.any():
            return (logits * 0.0).sum()

        flat_logits = logits[is_masked]
        x0_m = x0[is_masked]
        t_m = t.unsqueeze(1).expand_as(x0)[is_masked].float()

        probs = flat_logits.softmax(dim=-1)
        p_x0 = probs.gather(1, x0_m.unsqueeze(1)).squeeze(1).clamp(min=1e-8)
        p_msk = probs[:, self.mask_idx].clamp(min=1e-8)
    ...
    def sample(
        ...
        """Reverse diffusion from all-MASK at t=T to x₀."""
        ...
        for step in range(self.T, 0, -1):
            ...
            logits = self.denoiser(x, t_vec, pad_mask=None)
            probs_x0 = logits.softmax(dim=-1)
            ...
            if step == 1:
                ...
                flat = probs_x0[is_masked]
            else:
                t_val = float(step)
                posterior = probs_x0 / t_val
